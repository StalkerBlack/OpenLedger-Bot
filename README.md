...
Софт автоклеймер Daily Rewards и чек количества поинтов на счету
...
1. Софт актуален только для уже авторизованных аккаунтов
2. Софт работает на запросах, используя session token, который необходимо достать из консоли браузера каждого аккаунта

  👉🏻 Как достать токен? [Инструкция](https://teletype.in/@divinus/Xm8sYPnwCbv)

## 📄 Ввод своих данных  

### Все нужные данные необходимо указать в `accounts.xlsx` файл:  

1. **Name** - Любые имена , главное чтобы отличались друг от друга 
2. **Proxies** - прокси для каждого аккаунта. Формат: `http://login:password@ip:port`
3. **Token** - Token
